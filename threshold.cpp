...
int main()
{
    cv::Mat src = cv::imread("C:\\Users\\moamen\\Desktop\\2.png",0);
    cv::resize(src,src,cv::Size(500,600),0,0,cv::INTER_CUBIC);
    cv::Mat dst = src;

    localThreshold(src,dst,10,OTSU,BINARIZE);
//    otsu(src,dst,BINARIZE);
    cv::imshow("dst",dst);
    cv::waitKey(0);
    return 0;
}
