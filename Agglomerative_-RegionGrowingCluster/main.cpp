...
int main(int argc, char * argv[])
{
	...
	cv::Mat image = cv::imread("C:\\Users\\EGYPT_LAPTOP\\Desktop\\Paronamas\\unknown.png");
	

	RegioinGrowing(image, 100, 40, 70);
	Agglomerative(image);
	cv::Mat theModified = RGBToLUV(image);
	cv::imshow("alksd", theModified);
	cv::waitKey();
		...
		//std::vector<std::pair<std::pair<float, float>,float>>mm;
	...
	for (;;)
	{
		...
		if (pt.x != -1 && pt.y != -1)
		{
			...
			{
				QMessageBox mkr;
				mkr.setText("X is " + QString::number(pt.y) + " Y is " + QString::number(pt.x));
				mkr.exec();
			}
		}
		imshow("img", image);
	...
	cv::Mat img = cv::imread("C:\\Users\\EGYPT_LAPTOP\\Desktop\\SIFTTTTTTTTT\\sift-cpp\\imgs\\book.png");
	
	std::vector<Keypoint> kps = find_keypoints_and_descriptors(img,0.8,8,3,0.015,10,1.5,6);
	cv::Mat result = draw_keypoints(img, kps);
